...
public class PlacePhotoMetaInfoRepository implements PhotoMetaInfoRepository {
    ...
    public Page<PhotoMetaInfo> findAllByPlaceId(UUID ownerId, Pageable pageable) {
        List<PhotoMetaInfo> photos = dsl.select(Tables.PHOTOS)
                .from(Tables.PHOTOS)
                .join(Tables.PLACE_PHOTOS)
                .on(Tables.PLACE_PHOTOS.PHOTO_ID.eq(Tables.PHOTOS.ID))
                .where(Tables.PLACE_PHOTOS.OWNER_ID.eq(ownerId))
    ...
    public PhotoMetaInfo save(UUID ownerId, PhotoMetaInfo photoMetaInfo) {
        ...
        dsl.insertInto(Tables.PLACE_PHOTOS)
                .set(Tables.PLACE_PHOTOS.PHOTO_ID, photoMetaInfo.getId())
                .set(Tables.PLACE_PHOTOS.OWNER_ID, ownerId)
                .execute();
